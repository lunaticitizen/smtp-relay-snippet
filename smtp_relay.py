...
parser.add_argument("--server", default=None, help="SMTP server IPv4/IPv6 address")
...
if args.server:
    try:
        addr = ipaddress.ip_address(args.server)
    except ValueError:
        print(f"Error: '{args.server}' is not a valid IPv4 or IPv6 address", file=sys.stderr)
        sys.exit(1)
    host = str(addr)
